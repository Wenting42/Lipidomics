...
####################################################################################
# Script: FWL_lipidomics2.8.functions.R
...
initial.package <- c("BiocManager", "rlang", "devtools")
...
list.of.packages <- c( "FactoMineR", "factoextra", "scales", "magrittr", "ggrepel", "reshape2",
                       ...
                       "stringr", "readxl", "RCy3", "igraph", "tidyverse", "dplyr", "EnhancedVolcano")
...
InternalCheck <- function(internals, lipid){
  if(internals=="y"){
    ...
    internal_P.Nos <- sapply(internal_standards, function(x)addquotes(!!name2, !!x, "]"))
    internal_MainArea <- name3 <- sapply(internal_standards, function(x)addquotes(!!name3, !!x, "]"))
    grades <- sapply(internal_Grades, function(x)expr(-contains(!!!x)))
    number_of_p <- sapply(internal_P.Nos, function(x)expr(-contains(!!!x)))
    areas <- sapply(internal_MainArea, function(x)expr(-contains(!!!x)))
    updated_lipid <- lipid
    for( i in grades){
...
Input <- function(data){
  ...
  message("Please input the info of the experimental groups below and end with 'Enter'
        ...
        Sample names (replicate number from lipid search, e.g. S3): s1 s2 s3 s4 
...
ReplaceNa <- function(x, na.rm=FALSE){x=replace(x, x == "NaN", 1)}
...
TotalNormalClass <- function(data, n_control){
  ...
  for(i in 1:n_control){
    ...
    order_fc_long[[i]] <- fc_long[[i]] %>% 
      mutate(class= factor(class, levels = order_factor))  %>% 
      arrange(class) 
  }
...
#   # name2 <- readline("Please input the name for significant data (p value less than 0.05) file, e.g. A.B.sig.csv: ")
#   # write_csv(x=output2, file.path("data", name2))
#   # volcano plot
...
#      #         fill = "#f0f0f0", linetype="dashed", size=0.2)+
#     geom_point() +
#     geom_vline(xintercept = c(-fold_change, 1, fold_change), linetype = "dashed", colour = "black")
...
########################################################################
buildPathWay <- function(class){
  nodes <- data.frame(nodes = class, log2fc = rep(length(class), 1))
  edges <- data.frame(from=c("fatty acids", "LCBs", "Cer", "fatty acids", "G3P", "LPA", "PA", 
                      ...
                      to=c("LCBs", "Cer", "SM", "LPA","LPA", "PA", "DAG", "CDP-DAG", "TAG", 
                           ...
                           "LPS", "PC", "PS", "PG", "PI", "PE"))
  ig <- graph_from_data_frame(edges, directed = TRUE, vertices = class)
  return(ig)
}
...
# utility: plot SFA, MUFA and PUFA for each class
########################################################################
NormalizeGroup <- function(data){
  groups <- unique(data$Group)
  control <- check_group(groups)
  n <- length(groups)
  # 1) just fold change 
  control_values <- data %>% filter(Group == control) %>% select(Value) %>% unlist()
  values <- rep(control_values, n)
  data$Value <- data$Value/values
  # replace NaN with 0
  data$Value <- apply(data[, colnames(data)=="Value"], 1, function(x)ifelse(x == "NaN", 0, x))
  return(data)
}
