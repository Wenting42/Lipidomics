####################################################################################
# Script: fattyAcidsSaturation_analysis.2.2.2.r
# Author: Wenting Lyu
...
#####################################################################################
rm(list=ls())
source("fattyAcidsSaturation_analysis.FUNCTIONS.2.2.2.r")
...
p2 <- PlotTypes(dt_median, pars2) + 
  ...
  geom_bar(stat = "identity") 
print(p2)
ggsave(filename = "medianBased.stackplots.pdf", path="plot/", device="pdf")

# bar plots split by lipid class
pars3 <- c("TYPE", "mean", "Groups")
p3 <- PlotTypes(dt, pars3)  +  geom_bar(stat = "identity",  position=position_dodge()) 
print(p3)
ggsave(filename = "meanBased.fattyAcids.pdf", path="plot/", device="pdf")

# normalized the data for visualization
dt_norm<- PassToNormal(dt)
p4 <- PlotTypes(dt_norm, pars3)  +  geom_bar(stat = "identity",  position=position_dodge())
print(p4)
ggsave(filename = "meanBased.foldchange.pdf", path="plot/", device="pdf")


# percentage plots
p5 <- PlotTypes(dt, pars1) + labs(title = "mean based stackplot") + geom_bar(stat = "identity", position = "fill") 
print(p5)
ggsave(filename = "percentage.pdf", path="plot/", device = "pdf")
