####################################################################################
# Script: fattyAcidsSaturation_analysis.FUNCTIONS.2.2.2.r
# Author: Wenting
...
#                 source("fattyAcidsSaturation_analysis2.2.2.r") #
#####################################################################################
...
CalcGroup <- function(percent_info, groups){
  ...
  for (i in group_list){
    ...
    dt1 <-  dd %>% rowwise() %>% transmute(!!(medians[1]) := median(c(!!!syms(samples_SFA))),
                                           ...
                                           !!(sds[2]) := sd(c(!!!syms(samples_PUFA))),
                                           !!(sds[3]) := sd(c(!!!syms(samples_MUFA))))
    dd <- cbind(dd, dt1)
  }
  return(dd)
}
