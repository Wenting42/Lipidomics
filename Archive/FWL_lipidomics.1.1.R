...
message("If the file has extracted and unextracted samples, please input the Grades and APValue as guides. And if only one control sample need to delete, just type last sample one more time or something unexist for the second option. 
...
lipidCount <- lipidomics %>% 
              select(contains("Grade"), -contains("Grade[c]" ),
                     ...
                     contains("APValue"),
                     -contains(extracted.P.No), -contains(unextracted.P.No))  %>% 
...
totalClass <- function(data){
  total.plot <- ggplot(data, aes(x=class, fill=experiment.group, y=value)) + 
                ...
                coord_cartesian(ylim=c(1e5,1e13)) ## these y axis range parameter can be changed
  print(total.plot)
}

totalClass(total.class.long)
...
if(judge == "Y"){
    ...
    total.class.long <- total.class.long %>% filter(!class %in% class.remove)
}
...
eachClassPlots <- function(){
    ...
    class.sd <- aggregate(.~experiment.group, data=data.wide, function(x) sd(x)) %>%                  gather( Acyl, sd, -experiment.group)
    ...
    class.plot <- ggplot(class.long, aes(x=Acyl, fill=experiment.group, y=AreaValue)) + 
                  geom_bar(position="dodge", stat="identity") +
...
message("Please note that you NEED make contrast groups manually to Compare the difference between/among groups.
        e.g. compare B+C against A: A-(B+C)/2; A against B:  A-B; A+B against C+D: (A+B)/2-(C+D)/2, etc.")
...
replicate( n.comparisons, {
  ...
  volc.plot <- ggplot(input, aes(logFC, -log10(adj.P.Val))) + #volcanoplot with log2Foldchange versus pvalue
              ...
              scale_y_continuous(expand = c(0, 0), limits=c(0,15)) +       ###### setting the start?
