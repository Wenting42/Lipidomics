...
replicate( n.comparisons, {
  ...
  name1 <- readline("Please input the name for comparison data, e.g. G1.G2.csv: ")
  write.csv (x=toptable_OUTPUT1, file=sprintf(name))
  ...
  toptable_OUTPUT1.1 <- topTable(fit2, coef=1, adjust.method = 'fdr',
                        ...
                        as.data.frame()
  name1.1 <- readline("Please input the name for significant comparison data: ")
  write.csv (x=toptable_OUTPUT1.1, file=sprintf(name))
  # volcano plot
  fold.change <- as.numeric(readline("Please input the fold change treshold: "))
  input <- comparison
  ...
  ggplot(input, aes(logFC, -log10(adj.P.Val))) + #volcanoplot with log2Foldchange versus pvalue
