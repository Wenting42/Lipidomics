...
class_lipids <- filtered_lipidomics %>% 
...
unique_molec_names <- molec_names[which(size>1)]
# get duplicated lipid molecules
duplicate_molecs <- class_lipids[which(class_lipids$LipidMolec %in% unique_molec_names), ] %>% arrange(Class, LipidMolec, BaseRt)

# get 
# z <- y %>% group_by(Class, LipidMolec) %>% mutate(tt=max(BaseRt)-min(BaseRt)) %>% select(-contains("MainArea")) 
# 
# zz <- z %>% select(contains("MainArea"), -contains("MainArea[c"))
# 
# cc <- function(x){sapply(zz[,-1], class(x) <- as.numeric(x))}
# nam <- colnames(zz)[-(1:2)]
# z1 <- zz[9, -(1:2)] %>% as.numeric(.)


### remove artifical lipids (odd chains)
...
# 
# a <- filtered_lipidomics %>% filter(Class=="TG") 
...
# # odd in total is 11%
# # TG in total is 21.5%
# nrow(a)/nrow(filtered_lipidomics)
...
ggsave(filename = "background.pnd", path="plot/", device = "png")
...
# # loadTableData(path.data)
# 
# column <- sample(group_names[!group_names %in% control.group])
# control.points <- path.data %>% select(control.group)
# sizes <- c()
# 
