...
#lipidomics <- readxl::read_xlsx(file.choose(), col_types = "guess")
...
view(aggregate_lipids)
# sdata <- aggregate_lipids %>% filter(Class == "LPC") 
# sdd <- SumByGroup(ngroups, sample_info, sdata)
# kk <- sdata %>% select(Class, n, FA.name) %>% bind_cols(., sdd)
# dd <- spread(kk, Class, FA.name)
# mm <- data.frame(Group = rep(c("G1", "G2", "G3", "G4", "G5", "G6"), 3), pattern = c(rep(MUFA, 6)))

# Subtraction <- function(data, ..., na.rm=FALSE){
#   print("start")
#  vars <- ensyms(...) 
#  name <- enquos(...)
#  strings <- enexprs(...)
...
#  
#  print("momo")
#  a <- data %>% mutate_at(vars(!!!strings), function(x) x - !!control))
#  
 # a <- vars[[1]]
  #print(a)
 # vars <- enquo(v)
#a <- data %>% mutate_at(!!vars :=  !!vars + 1000000)
