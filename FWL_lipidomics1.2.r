...
#           
# Warning: If you want to change parameters please DO NOT change png format to pdf which 
...
ggsave(filename = plot.name, path = 'plot/', device = "png", dpi=300)
...
retention.all.plot <- ggplot(data=retention.data, 
                      ...
                      facet_grid(.~Class) +
                      labs("Abundance VS. Retention time", 
                           x="All samples (log10(MainArea))", y="Retention time")
...
for(i in 1:length(sample.index)){
  range     <- sample.index[i]
  plot.name <- paste("pairs.plot.", i, ".oldversion.pdf",sep="")
  path      <- file.path("plot/", plot.name)
...
# the y axis range can be editted by changing the parameter in "coord_cartesian(ylim=c(1e5,1e13))"
  ...
  ggsave(filename="total.class.1.png",path = 'plot/', device = "png", dpi = 300)
  ...
  total.normalization.plot <- ggplot(normal.log2.data, aes(x=reorder(class, value), fill=experiment.group, y=value)) +
   ...
   ggsave(filename="total.class.foldchange.png",path = 'plot/', device = "png", dpi = 300)
   ...
   n.control <- readline("Please input how many controls for normalizations you want: ")
   control.group <- c()
   for(i in 1:n.control){
      control.group[i] <- readline("Please input a group as control group for normalization, e.g. A : ")
    ...
    normal.log2.matrix[1, -1] <- rep(1, ncol(normal.matrix)-1)
...
for(i in 1:length(classes)){
  ...
  if(n.observations <= n.bar){
    ...
    print(p1)
    ggsave(filename = paste(pick.class, ".pdf", sep=""), path = 'plot/', device = "pdf",  dpi=300)
  }else{
    #class.long$bins <- 1:n.observations
    if(n.bar %% ngroups != 0){
       n.bar <- (n.bar%/% ngroups)*ngroups
      }
    ...
    for(i in 0:n.facet){
      ...
      if(i <n.facet){
        ...
        row.range <- (n.bar*i+1):(n.bar*(i+1))
        data <- class.long %>% slice(row.range) 
        p2 <- classPlot(data)
        print(p2)
        ggsave(filename = paste(pick.class, ".", i+1, ".pdf", sep=""), path = 'plot/', device = "pdf")
      }else{
          #class.long$bins[(n.bar*i):nrow(class.long)] <- rep(m, nrow(class.long)%%n.bar)
          row.range <- (n.bar*i+1):nrow(class.long)
          data <- class.long %>% slice(row.range) 
          p2 <- classPlot(data)
          print(p2)
          ggsave(filename = paste(pick.class, ".", i, ".pdf", sep=""), path = 'plot/', device = "pdf")
          }
    }
  }
  ...
  #   
  # }
  # p1 <- classPlot(class.long) + facet_wrap(~bins, ncol=1)
  # print(p1)
  ...
  ###### change to facet_grid (bins!)
  
  
  
  # diplay graph in separate windows.
  # If you are using mac, don't delete this line of code otherwirse might induce some errors.
  #quartz()
  ...
  # 
  # print(class.plot)
  #ggsave(filename = paste(pick.class, ".png", sep=""), path = 'plot/', device = "png",  dpi=300, type = c("cairo", "cairo-png", "Xlib", "quartz"))
 }
...
replicate( n.comparisons, {
  ...
  ggsave(filename = plot.name, path = 'plot/', device = "png", width=15, height=15, dpi=300)
  #dev.off()
  ...
  volc.plot.color <- ggplot(input.lip, aes(logFC, -log10(adj.P.Val)), fill=lipclass) + #volcanoplot with log2Foldchange versus pvalue
    ...
    scale_size_continuous(range = c(0.25,2.5), breaks=c(10, 20, 30))+ #add size limit
    scale_color_manual(values=c("darkorange3","dodgerblue3",  'chartreuse4', '#c51b8a', '#756bb1','#636363'), drop=FALSE)+
    #ggtitle ('Total Lipid species') +
    #xlim(c(-2.7, 2.7)) + ylim(c(0, 2)) + #set the x,y axis limits.
    xlab("Log2 fold change ") + ylab("-Log10 (q value)") + 
    theme_bw()+ # remove background
