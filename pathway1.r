...
internal.check <- readline("Please type Y/N: ")
if(internal.check!="N"){
...
Input <- function(filtered.lipidomics){
  ...
  if(str_to_lower(group.condition) == "n"){
    message("Continue")
  }else{
    total.info <- inputGroups(ngroups)
    glimpse(total.info)
  ...
  message("Take a look at the sample info and its position information in the file below")
  glimpse(sample.info)
...
pca.pairs.plots <- function(sample.info, group.names, filtered.lipidomics){
...
info.list <-  pca.pairs.plots(sample.info, group.names, filtered.lipidomics)
...
for(i in 1:length(classes)){
  ...
  if(n.observations <= n.bar){
    ...
    print(p1)
    ggsave(filename = paste(pick.class, ".pdf", sep=""), path = 'plot/classes', device = "pdf", width=15, height=15, dpi=300)
  }else{
    ...
    for(i in 0:n.facet){
      ...
      if(i <n.facet){
        ...
        p2 <- classPlot(data)
        print(p2)
        ggsave(filename = paste(pick.class, ".", i+1, ".pdf", sep=""), path = 'plot/classes/', device = "pdf")
      }else{
        ...
        p2 <- classPlot(data)
        print(p2)
        ggsave(filename = paste(pick.class, ".", i, ".pdf", sep=""), path = 'plot/classes/', device = "pdf")
      }
    }
  }
}
...
replicate( n.comparisons, {
  ...
  volc.plot.color <- ggplot(input.lip, aes(logFC, -log10(adj.P.Val)), fill=lipclass) + #volcanoplot with log2Foldchange versus pvalue
    ...
    #ggtitle ('Total Lipid species') +
    #xlim(c(-2.7, 2.7)) + ylim(c(0, 2)) + #set the x,y axis limits.
    xlab("Log2 fold change of groups contrast") + ylab("-Log10 (q value)") + 
    theme_bw()+ # remove background
    theme(line=element_blank())+
    #scale_y_continuous(expand = c(0, 0), limits=c(0,15)) +       ###### start at 0
    #geom_text_repel(data=points, aes(label=rownames(points)), size=2)#adding text for the FDR points.
    geom_text_repel(data=sig.points, aes(label=rownames(sig.points)), size=2)   +
    geom_text_repel(data=fold.points, aes(label=rownames(fold.points)), size=2) 
  #geom_rug()
