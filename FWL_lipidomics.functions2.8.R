...
initial.package <- c("BiocManager", "rlang")
...
list.of.packages <- c( "FactoMineR", "factoextra", "scales", "magrittr", "ggrepel", "reshape2",
                       ...
                       "stringr", "readxl", "RCy3", "igraph", "tidyverse", "dplyr")
...
########################################################################
 buildPathWay <- function(nodes, size){
   nodes <- data.frame(nodes, age=size)
   edges <- data.frame(from=c("fatty acids", "LCBs", "Cer", "fatty acids", "G3P", "LPA", "PA", 
                       ...
                       to=c("LCBs", "Cer", "SM", "LPA","LPA", "PA", "DAG", "CDP-DAG", "TAG", 
                            ...
                            "LPS", "PC", "PS", "PG", "PI", "PE"))
   ig <- graph_from_data_frame(edges, directed = TRUE, vertices = nodes)
   return(ig)
 }
